...
export function PrintersPage() {
  const { printers, isAdmin, addPrinter, updatePrinter, removePrinter } = useData();
  ...
  const handleSubmit = async (input: PrinterInput) => {
    if (editing) {
      await updatePrinter(editing.id, input);
      showToast("Đã cập nhật máy in thành công.");
      setEditing(null);
    } else {
      await addPrinter(input);
  ...
  const handleDeleteConfirm = async () => {
    if (!deleting) return;
    await removePrinter(deleting.id);
  ...
  return (
    <div>
      ...
      <div className="card overflow-hidden">
        <PrinterTable items={printers} canManage={isAdmin} onEdit={setEditing} onDelete={setDeleting} />
      </div>
